...
TGAImage::TGAImage(int w, int h, int bpp) : data(NULL), width(w), height(h), bytespp(bpp)
{
    ...
    memset(data, 0, nbytes); // params: destination, value, size
}
...
TGAColor generateRandomColor(std::mt19937 &gen)
{
    ...
    unsigned char r = dis(gen);                  // Random red
    unsigned char g = dis(gen);                  // Random green
    unsigned char b = dis(gen);                  // Random blue
    return TGAColor(r, g, b, 255);               // Fully opaque
}
